...
locals {
  crdb_haproxy_prefix = format("%s-%02d", var.crdb_haproxy_name_prefix, (var.vsphere_cluster_index + 1))
  crdb_haproxy_ip_offset = local.crdb_client_ip_offset + var.crdb_client_vm_per_cluster
}

resource "vsphere_virtual_machine" "crdb_haproxy_vm" {
  count = var.crdb_haproxy_vm_per_cluster

  name = format("%s-%02d", local.crdb_haproxy_prefix, (count.index + 1))
  guest_id = data.vsphere_virtual_machine.vs_vm_template.guest_id
  hardware_version = var.crdb_haproxy.hw_version
  ...
  # Hardware Configuration
  num_cpus = var.crdb_haproxy.cpu_count
  memory = var.crdb_haproxy.memory_gb * 1024
  ...
  disk {
    label = format("%s-%02d-os-disk00", local.crdb_haproxy_prefix, (count.index + 1))
    size = var.crdb_haproxy.os_disk_gb
    storage_policy_id = data.vsphere_storage_policy.vs_ds_policy.id
    unit_number = 0
  }
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = format("%s-%02d", local.crdb_haproxy_prefix, count.index + 1)
        domain = var.vsphere_cluster.vs_vm_domain
      }

      network_interface {
        ipv4_address = var.vsphere_cluster.vs_dvs_pg_1_ipv4_ips[(local.crdb_haproxy_ip_offset + count.index)]
        ipv4_netmask = regex("/([0-9]{1,2})$", var.vsphere_cluster.vs_dvs_pg_1_ipv4_subnet)[0]
      }
...
# CRDB HA Proxy Anti-affinity rules
resource "vsphere_compute_cluster_vm_anti_affinity_rule" "crdb_haproxy_anti_affinity_rule" {
  count = var.crdb_haproxy_vm_anti_affinity ? 1 : 0
  name                = format("%s-anti-affinity-rule", var.crdb_haproxy_name_prefix)
  compute_cluster_id  = data.vsphere_compute_cluster.vs_cc.id
  virtual_machine_ids = vsphere_virtual_machine.crdb_haproxy_vm.*.id
}
...
locals {
  ...
  crdb_ip_offset = local.crdb_haproxy_ip_offset + var.crdb_haproxy_vm_per_cluster
}
